...
static void perm_data_free_rcu(struct rcu_head *head)
{
    struct perm_data *p = container_of(head, struct perm_data, rcu);
    kfree(p);
}

int ksu_set_app_profile(struct app_profile *profile, bool persist)
{
    ...
    list_for_each_entry (p, &allow_list, list) {
        ...
        if (profile->current_uid == p->profile.current_uid &&
            ...
            call_rcu(&p->rcu, perm_data_free_rcu);
            goto out;
        }
    }
                         ...
                         void *data)
{
    ...
    list_for_each_entry_safe (np, n, &allow_list, list) {
        ...
        if (!is_preserved_uid && !is_uid_valid(uid, package, data)) {
            ...
            call_rcu(&np->rcu, perm_data_free_rcu);
            if (likely(uid <= BITMAP_UID_MAX)) {
